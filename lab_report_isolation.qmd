...
histogram_tumor_dna <- ggplot(data = qty_tumor, mapping = aes(x = qty_dna)) +
  ...
  geom_vline(xintercept = 500, colour = 'green') + geom_vline(xintercept = 260, colour = 'orange')  +
...
```
<span style="color: green;">Green line</span> - 500ng - DNA quantity allowing for exectuion of RRBS (methylation profiling) and WES with a safety margin - achieved in `r perc(qty_tumor$qty_dna>=500)` samples
